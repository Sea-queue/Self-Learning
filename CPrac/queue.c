...
queue_t *queue_new(unsigned int capacity) {
  ...
  q->size = 0;
  q->capacity = capacity;
  //second way to allocate memory: long data_array[capacity];
  long *data_array= (long*)malloc(sizeof(long)*capacity);
  q->data = data_array;
  return q;
}
...
void queue_enqueue(queue_t *q, long item) {
  ...
  //if enqueue an item at the index of [capacity - 1]
  //and if the there is empty space after insert, update back to 0;
...
long queue_dequeue(queue_t *q) {
  assert(q != NULL);
  assert(q->size > 0);
  long front_item = q->data[q->front];
  // if dequeue the item at the index of [capacity - 1] 
  // the front should points to the 0 index.
  if (q->front == q->capacity - 1){
	  q->front = 0;
  }
  // else just increment the front by one
  else {
  	q->front += 1;
  }
  q->size -= 1;
